...
# for i in range(6):
#     calib_config['param_name'][i] = 'base_placement_%d' % (i+1)
...
if dataSet == "sample":
    ...
    #     var_sample, q_sample, model, data, calib_config)
...
def cost_func(var, coeff, q, model, data, calib_config, PEEm):
    ...
    res_vect = np.append(
        (PEEm - PEEe), np.sqrt(coeff) * var[6 : -calib_config["NbMarkers"] * 3]
    )
    # res_vect = (PEEm - PEEe)
    return res_vect
...
PEEe_sol = calc_updated_fkm(model, data, res, q_LM, calib_config, verbose=1)
...
uncalib_res[:3] = res[:3]
uncalib_res[-3:] = res[-3:]
PEEe_uncalib = calc_updated_fkm(model, data, uncalib_res, q_LM, calib_config)
...
PEE_xyz = delta_PEE.reshape((calib_config["NbMarkers"] * 3, calib_config["NbSample"]))
...
if calib_config["NbMarkers"] == 1:
    ...
    for i in range(calib_config["NbMarkers"]):
        ax1[i].bar(np.arange(calib_config["NbSample"]), PEE_dist[i, :], colors[i])
...
PEEm_LM2d = PEEm_LM.reshape((calib_config["NbMarkers"] * 3, calib_config["NbSample"]))
PEEe_sol2d = PEEe_sol.reshape((calib_config["NbMarkers"] * 3, calib_config["NbSample"]))
for i in range(calib_config["NbMarkers"]):
    ax2.scatter3D(
        PEEm_LM2d[i * 3, :],
        PEEm_LM2d[i * 3 + 1, :],

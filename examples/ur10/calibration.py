...
def _run_calibration(
    ...
    *,
    ...
    if asset_id or operator:
        instance = dict(ur10_calib.calib_config.get("instance") or {})
        if asset_id:
            instance["asset_id"] = asset_id
        if operator:
            instance["operator"] = operator
        ur10_calib.calib_config["instance"] = instance
    ur10_calib.initialize()
    result = ur10_calib.solve(
    ...
    if html_report or geometric_calibration_yaml or archive:
        run_dir = compute_run_dir(ur10_calib)

    if html_report:
        ur10_calib.export_html_report(output_path=str(run_dir / "report.html"))

    if geometric_calibration_yaml:
        try:
            export_geometric_calibration_yaml(
                ur10_calib, str(run_dir / "master_calibration.yaml"),
                header_comment="UR10 calibration -- full",
            )
            export_geometric_calibration_yaml(
                ur10_calib,
                str(run_dir / "master_calibration_conservative.yaml"),
                min_sigma=2.0,
                header_comment="UR10 calibration -- conservative (>=2sigma)",
            )
    ...
    if archive:
        archive_run(ur10_calib, run_dir)

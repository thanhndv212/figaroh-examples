...
reference run (RMSE 6.46 mm, MAE 4.94 mm, 94 samples).
...
# BaseCalibration and this class fit models the same way but the quality
# metrics below (condition number, parameter uncertainty, correlation,
# per-DOF stats) live as BaseCalibration instance methods. Rather than
# rewriting the validated TiagoProCalibration.solve() to inherit from
# BaseCalibration, these are the same formulas
# (BaseCalibration._compute_condition_number/_compute_per_dof_stats/
# calc_stddev/_compute_parameter_correlation) reimplemented as free
# functions, then attached to the calib instance so figaroh's actual
# report/provenance/archive code (unmodified) can consume it exactly as
# it would a BaseCalibration instance.
...
def _compute_per_dof_stats(residuals, PEE_measured, n_dofs, n_samples) -> dict:
    ...
    orient_rows = (
        residuals_2d[3:6, :] if n_dofs >= 6 else np.zeros((3, n_samples))
    )
    pos_rmse = float(np.sqrt(np.mean(np.sum(pos_rows ** 2, axis=0)))) * 1000
    orient_rmse = (
        float(np.sqrt(np.mean(np.sum(orient_rows ** 2, axis=0)))) * 180 / np.pi
    )
    pos_max = float(np.max(np.sqrt(np.sum(pos_rows ** 2, axis=0)))) * 1000
    orient_max = (
        float(np.max(np.sqrt(np.sum(orient_rows ** 2, axis=0)))) * 180 / np.pi
    )

    return {
        "dof_names": dof_names, "mean": means, "std": stds, "rmse": rmses,
        "max_abs": max_abs, "r_squared": r_squareds,
        "overall": {
            "pos_rmse_mm": pos_rmse, "orient_rmse_deg": orient_rmse,
            "pos_max_mm": pos_max, "orient_max_deg": orient_max,
        },
    }
...
def main():
    ...
    parser.add_argument("--urdf", default=str(_URDF_DEFAULT), help="Path to the Tiago Pro URDF")
